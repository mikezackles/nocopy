...
SCENARIO("archive") {
  ...
  GIVEN("a versioned archive") {
    THEN("the correct fields should be present") {
      ...
      REQUIRE(!m::v<0>::has(m::nested_field_v<0>));
      ...
      REQUIRE(!m::v<1>::has(m::nested_field_v<1>));
      ...
      REQUIRE(!m::v<2>::has(m::nested_field_v<2>));
      ...
      REQUIRE(!m::v<3>::has(m::nested_field_v<3>));
      ...
      REQUIRE(!m::v<4>::has(m::nested_field_v<3>));
      REQUIRE(m::v<4>::has(m::nested_field_v<4>));
      ...
      REQUIRE(!m::v<5>::has(m::nested_field_v<4>));
      REQUIRE(m::v<5>::has(m::nested_field_v<5>));
    }
  }
}
